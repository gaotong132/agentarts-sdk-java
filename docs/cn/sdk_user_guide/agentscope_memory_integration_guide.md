...
sequenceDiagram
    ...
    participant L as LongTermMemory
    participant M as AgentArts 消息面
    participant MM as 模型

    C->>A: call
    Note over A,S: 调用开始 读短期
    A->>S: get
    S->>M: listMessages 读回对话
    M-->>S: 消息列表
    S-->>A: 重建 AgentState.context

    Note over A,H: PRE_CALL
    H->>L: retrieve
    L->>M: searchMemories 召回
    M-->>L: 长期记忆
    L-->>A: 注入 system
    ...
    A->>S: save 含新 context
    S->>M: delta addMessages forceExtract
    Note over M: 一次写入 短期可查 加 触发长期抽取

    Note over A,H: POST_CALL
    H->>L: record
    Note over L: no-op 短期已写加抽取 不重复
    A-->>C: 回复
```

> 关键：`AgentArts 消息面`（`M`）同时承担**短期存档**（`listMessages` 读回 / `addMessages` 写入）与
