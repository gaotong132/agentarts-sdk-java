...
每个阶段使用 `/goal` 驱动自主实现。每个 Goal 必须满足三个维度：**编译通过** + **单测全绿** + **Python 一致性验证**。
...
RequestResult 字段与 Python RequestResult 一一对应（success/statusCode/data/error/headers/streaming），或 20 轮后停止
```
...
Identity Python 一致性验证：
  ...
  TokenPoller 支持 IN_PROGRESS/COMPLETED/FAILED 状态和超时机制，或 20 轮后停止
```
...
验证方法签名和参数名与 Python 对应方法一一对应，或 20 轮后停止
```
...
模板系统：basic（Java handler）/ agentscope（ReActAgent + StateStore）/ docker（eclipse-temurin:17-jre），或 20 轮后停止
```
...
验证所有接口方法的行为与 agentscope 内置实现（InMemoryAgentStateStore 等）一致，或 20 轮后停止
```
...
全部模块 mvn test 总测试数 ≥130，或 15 轮后停止
```
