...
| E2E 测试 | 76 | 调用云服务 API（`agentarts-sdk-tests` 模块下 `e2e` 包） |

E2E 测试以 Python SDK 的 `tests/integration/`（`feature/test` 分支，pinned commit `2f64a4b`）为可信基准实现，**用例一一对应，Java 覆盖范围不低于 Python**（Java 额外包含 7 个 Memory 状态存储用例）。
...
> Python 用 `pytest.mark.xfail(run=False)` 标记为预期失败且不执行；Java 在 `@BeforeAll` 尝试创建，失败则各测试跳过断言（标记为预期失败）。根因相同：IAM trust_policy "malformed policy document"。
...
| # | Python | Java | 状态 |
|---|---|---|---|
| 1 | `test_find_agent_by_name` | `testFindAgentByName` | 🟡 Python `@skip`；Java 在 `@BeforeAll` 尝试创建，失败则跳过断言（后端要求 `artifact_source_config`） |
...
| Runtime | 18 | Agent/Endpoint 控制面 CRUD（前置失败即跳过）、数据面 session（计费）、本地 RuntimeApp（ping/invocations/SSE/WebSocket） |
...
2. **Runtime Agent 生命周期（前置失败即跳过）**：后端要求 `artifact_source_config`（已构建镜像）+ `identity_configuration`，最小 payload 创建会被拒。Python 整体 `@skip`；Java 在 `@BeforeAll` 尝试，失败则各测试跳过断言。提供可部署 artifact 后可解除。
3. **计费层（L3）默认跳过**：CodeInterpreter session、Runtime invoke 会产生实际费用，需显式 `AGENTARTS_TEST_RUN_BILLABLE=1` + 预置资源。
